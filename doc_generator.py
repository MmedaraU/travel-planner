...
def generate_itinerary_doc(executive, items, destination, trip_id, trip_budget,
    ...
    sub = doc.add_paragraph(f'Destination: {destination}   |   Timezone: {executive["timezone"]}')
    ...
    for item in items:
        ...
        end_str = datetime.fromisoformat(item['datetime_end']).strftime('%H:%M') if item['datetime_end'] else 'TBD'
    ...
    if trip_budget > 0:
        ...
        doc.add_paragraph(f"Budget: {currency_symbol}{trip_budget:.2f}  |  Remaining: {currency_symbol}{remaining:.2f}")
    
    # Footer
    footer = doc.add_paragraph()
    footer.alignment = WD_ALIGN_PARAGRAPH.CENTER
    footer_run = footer.add_run(f'Generated on {datetime.now().strftime("%B %d, %Y at %I:%M %p")}  |  Currency: {currency_code}')
...
def generate_executive_profile_doc(profile_data, currency_symbol="$"):
    """Generate a .docx executive profile. Returns BytesIO stream."""
    doc = Document()
    ...
    footer = doc.add_paragraph()
    footer.alignment = WD_ALIGN_PARAGRAPH.CENTER
    footer_run = footer.add_run(f'Generated on {datetime.now().strftime("%B %d, %Y at %I:%M %p")}')
    footer_run.font.size = Pt(9)
    footer_run.font.italic = True
...
def generate_spending_report_doc(filter_name, summary_data, start_date, end_date, currency_symbol="$"):
    ...
    doc.add_paragraph(f"Total Confirmed (Booked): {currency_symbol}{total_confirmed:,.2f}")
    doc.add_paragraph(f"Total Estimated (Quoted): {currency_symbol}{total_estimated:,.2f}")
    doc.add_paragraph()
    ...
    headers = ["Executive", "Company", "Destination", "Budget", "Total Spent", "Confirmed", "Status"]
