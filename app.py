...
# =========================================================
# RESET FORM FLAG PROCESSING (runs before any widgets are rendered)
# =========================================================
if st.session_state.get("reset_form", False):
    # Reset trip planner widget keys to defaults
    defaults = {
        "trip_purpose_input": "",
        "departure_city_input": "",
        "departure_region_input": "",
        "departure_country_input": "",
        "trip_display_currency": "USD",
        "trip_base_currency": "USD",
        "display_exchange_rate": 1.0,
        "budget_currency_label": "Base Currency (USD)",
        "trip_budget_input": 0.0,
    }
    for key, default in defaults.items():
        st.session_state[key] = default
    # Also clear trip-related session keys
...
if profile:
    with st.sidebar.expander("📋 Quick Profile", expanded=False):
        ...
        if st.button("👤 View Full Profile"):
            # Switch to Executive Management tab and open edit form
...
# =========================================================
# MAIN AREA: TABS (using st.tabs with session state control)
# =========================================================
tab_names = [
    "✈️ Trip Planner",
...
# Initialize show_edit_form to False (closed by default)
if "show_edit_form" not in st.session_state:
    st.session_state.show_edit_form = False

# Create tabs with a key to control active tab
tabs = st.tabs(tab_names, key="tab_control")
...
for i, tab in enumerate(tabs):
    with tab:
        if i == 0:
            ...
            # --- Clear form function (uses flag to reset on next run) ---
            def clear_trip_form():
                """Reset all trip-related session state to clear the form."""
                st.session_state["reset_form"] = True
                # Clear trip data keys immediately
                st.session_state.pop("current_trip_id", None)
                st.session_state.pop("trip_destination_summary", None)
                st.session_state["trip_stops"] = []
                st.rerun()
            ...
            if is_editing and trip_data:
                ...
                if (
                    "trip_stops" not in st.session_state
                    or not st.session_state["trip_stops"]
                ):
            ...
            with st.expander("➕ Add Destination Stop"):
                ...
                if st.button(
                    "➕ Add Stop", key="add_stop_button", disabled=not is_draft
                ):
            ...
            if "current_trip_id" in st.session_state:
                ...
                # --- Define the add-item form function (accessible to both branches) ---
                def render_add_item_form(
                    ...
                    with st.form(key=f"add_item_form_{key_suffix}"):
                        ...
                        with cols[1]:
                            desc = st.text_input(
                                "Description", key=f"item_desc_{key_suffix}"
                            )
                        with cols[2]:
                        ...
                        with cols[3]:
                            dt_end = st.datetime_input(
                                "End Time",
                                value=datetime.now(),
                                key=f"item_end_{key_suffix}",
                            )

                        col_loc, col_cost, col_curr, col_conf = st.columns(4)
                        with col_loc:
                        ...
                        with col_curr:
                            ...
                            # Determine the chosen currency code
                            chosen_currency = trip_display_currency if "Display" in currency_choice else trip_base_currency
                        with col_conf:
                        ...
                        if st.form_submit_button("Add to Itinerary"):
                            if desc and dt_start:
                                existing_dupes = (
                                    duplicate_detection.find_duplicate_item(
                                        trip_id,
                                        desc,
                                        dt_start.isoformat(),
                                ...
                                if existing_dupes:
                                    ...
                                    if not st.checkbox(
                                        "Add anyway?",
                                        key=f"force_add_item_{key_suffix}",
                                    ):
                                        st.stop()
                                # Compute snapshot rate based on chosen currency
                ...
                # --- Trip Management ---
                st.divider()
                col_title, col_status, col_delete, col_duplicate, col_template = (
                    st.columns([3, 1, 1, 1, 1])
                )
                with col_title:
                    st.subheader(
                        f"📋 Current Itinerary: {trip_data.get('purpose', '')}"
                    )
                with col_status:
                    ...
                    if current_status == "draft":
                        ...
                        if st.button("📄 Mark as Final", key="finalize_trip"):
                            ...
                            st.rerun()
                        if st.button(
                            "↩️ Revert to Draft", key="revert_to_draft_approved"
                        ):
                ...
                with col_template:
                    if st.button("📋 Save as Template", key="save_template_btn"):
                        if stops or items:
                            ...
                            st.warning(
                                "This trip has no stops or items to save as a template."
                            )
                ...
                if st.session_state.get("show_save_template", False):
                    ...
                    with col_name:
                        template_name = st.text_input(
                            "Template Name*",
                            value=f"{trip_data.get('purpose', '')} Template",
                        )
                    with col_desc:
                ...
                if items:
                    ...
                    with col3:
                        st.metric(
                            "📊 Total Spend (Base)",
                            f"{base_symbol}{total_all_base:,.2f}",
                        )
                    with col4:
                    ...
                    with col8:
                        st.metric(
                            ...
                            delta_color=(
                                "inverse" if remaining_display < 0 else "normal"
                            ),
                        )
                        ...
                        if st.session_state.get(f"editing_item_{item['id']}", False):
                            with st.expander(
                                ...
                                with st.form(key=f"edit_item_form_{item['id']}"):
                                    ...
                                    # Cost and currency
                                    col_cost1, col_curr1, col_conf1 = st.columns(3)
                                    with col_cost1:
                                    ...
                                    with col_curr1:
                                        # Pre-select based on cost_currency
                                        current_currency = item.get("cost_currency", trip_display_currency)
                                        currency_options = [
                                            f"Display ({trip_display_currency})",
                                            f"Base ({trip_base_currency})"
                                        ]
                                        default_idx = 0 if current_currency == trip_display_currency else 1
                                        ...
                                        e_chosen_currency = trip_display_currency if "Display" in e_currency_choice else trip_base_currency
                                    with col_conf1:
                ...
                # --- ADD NEW ITEM (always shown, regardless of items) ---
                st.divider()
                    ...
                    if is_draft:
                        if st.button("🚀 Update Draft", key="update_trip_bottom"):
                            if trip_purpose and st.session_state["trip_stops"]:
                                ...
                                stop_cities = [
                                    stop["city"]
                                    for stop in st.session_state["trip_stops"]
                                ]
                                dest_summary = " → ".join(stop_cities)

                                existing_trips = (
                                    duplicate_detection.find_duplicate_trips(
                                        trip_exec_id,
                                        trip_purpose,
                                        overall_start,
                                        overall_end,
                                ...
                                st.success(
                                    f"Trip '{trip_purpose}' updated successfully!"
                                )
            ...
            # --- If a profile exists, show either edit form (collapsible) or view-only summary ---
            if profile:
                # Edit form inside an expander, controlled by show_edit_form
                with st.expander("✏️ Edit Executive", expanded=st.session_state.get("show_edit_form", False)):
                    st.subheader(f"Editing: {profile['name']}")
                    ...
                    with col_save:
                        if st.button("💾 Save Changes", key="save_exec_changes"):
                            ...
                            st.session_state.show_edit_form = False  # Collapse after save
                            st.rerun()
                    with col_cancel:
                ...
                # View-only summary (always visible)
                ...
                # --- No profile – show Add Executive form in a collapsed expander ---
            ...
            # The spending summary uses the modified database function that converts all costs to base currency
                    ...
                    with st.container():
                        ...
                        with col9:
                            if st.button("📂", key=f"open_trip_dash_{trip['trip_id']}"):
                                ...
                                st.session_state.tab_control = (
                                    0  # Switch to Trip Planner
                                )
