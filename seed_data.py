...
# ------------------------------------------------------------
companies = [
    {"name": "Nexus Ventures", "cost_center": "CC-2026-TECH", "policy": "Business class for >6hr flights."},
    {"name": "Apex Financial", "cost_center": "CC-2026-FIN", "policy": "Economy only, no exceptions."},
]
...
for comp in companies:
    ...
    if comp["name"] not in existing_names:
        ...
        # Get the ID of existing company
        for c in existing:
            if c[1] == comp["name"]:
...
for exec_data in executives:
    ...
    # Check if executive already exists by name + company
...
# ------------------------------------------------------------
memberships = [
    {"exec": "Sarah Chen", "category": "airline", "program": "Delta SkyMiles", "number": "DL-4567890"},
    {"exec": "Sarah Chen", "category": "hotel", "program": "Hilton Honors", "number": "HH-9876543"},
    {"exec": "Marcus Sterling", "category": "airline", "program": "United MileagePlus", "number": "UA-5556667"},
    {"exec": "Marcus Sterling", "category": "car", "program": "Hertz Gold", "number": "HZ-1234567"},
    {"exec": "Priya Sharma", "category": "airline", "program": "Cathay Pacific", "number": "CX-2345678"},
    {"exec": "Priya Sharma", "category": "hotel", "program": "Accor Live Limitless", "number": "ALL-8765432"},
]
...
# 6. CUSTOM CATEGORIES
# ------------------------------------------------------------
extra_cats = ["Conference", "Site Visit", "Workshop"]
...
# ------------------------------------------------------------
# 7. HELPER FUNCTION: Create a trip with stops & items
# ------------------------------------------------------------
def create_trip_with_details(exec_name, purpose, status, stops_data, items_data, budget=5000):
    ...
    # Check if a trip with same purpose and exec exists
    # We'll just create a new one every time
    trip_id = db.create_or_get_trip(exec_id, dest_summary, overall_start, overall_end, purpose)
    ...
    print(f"✅ Created trip: {purpose} ({status}) for {exec_name}")
...
# ------------------------------------------------------------
# 9. TRIP 1: DRAFT – Sarah Chen (Nexus Ventures)
# ------------------------------------------------------------
stops_sarah = [
    {"city": "London", "country": "United Kingdom", "region": "England",
     "start_date": now + timedelta(days=10), "end_date": now + timedelta(days=12)},
    {"city": "Paris", "country": "France", "region": "Île-de-France",
     "start_date": now + timedelta(days=13), "end_date": now + timedelta(days=15)},
]
items_sarah = [
    {"type": "Flight", "description": "BA 178 London → Paris",
     "start": now + timedelta(days=13, hours=8), "end": now + timedelta(days=13, hours=9, minutes=30),
     "location": "LHR", "cost": 120.50, "confirmed": False, "code": "BA178-ABC", "notes": "Economy"},
    {"type": "Hotel", "description": "Marriott Champs-Élysées",
     "start": now + timedelta(days=13, hours=14), "end": now + timedelta(days=15, hours=11),
     "location": "Paris", "cost": 450.00, "confirmed": False, "code": "MAR-4567", "notes": "Standard room"},
]
...
# ------------------------------------------------------------
# 10. TRIP 2: APPROVED – Marcus Sterling (Apex Financial)
# ------------------------------------------------------------
stops_marcus = [
    {"city": "New York", "country": "United States", "region": "NY",
     "start_date": now + timedelta(days=20), "end_date": now + timedelta(days=22)},
    {"city": "Dubai", "country": "United Arab Emirates", "region": "Dubai",
     "start_date": now + timedelta(days=23), "end_date": now + timedelta(days=25)},
]
items_marcus = [
    {"type": "Flight", "description": "Emirates EK 202 New York → Dubai",
     "start": now + timedelta(days=23, hours=7), "end": now + timedelta(days=23, hours=14),
     "location": "JFK", "cost": 1400.00, "confirmed": True, "code": "EK202-456", "notes": "Business Class"},
    {"type": "Meeting", "description": "Finance Summit Keynote",
     "start": now + timedelta(days=24, hours=9), "end": now + timedelta(days=24, hours=11),
     "location": "Dubai World Trade Centre", "cost": 0, "confirmed": True, "code": "", "notes": "Main stage"},
]
create_trip_with_details(
...
# ------------------------------------------------------------
# 11. TRIP 3: FINAL – Priya Sharma (Apex Financial)
# ------------------------------------------------------------
stops_priya = [
    {"city": "Singapore", "country": "Singapore", "region": "Singapore",
     "start_date": now + timedelta(days=5), "end_date": now + timedelta(days=7)},
    {"city": "Bangkok", "country": "Thailand", "region": "Bangkok",
     "start_date": now + timedelta(days=8), "end_date": now + timedelta(days=10)},
]
items_priya = [
    {"type": "Flight", "description": "SQ 116 Singapore → Bangkok",
     "start": now + timedelta(days=8, hours=8), "end": now + timedelta(days=8, hours=10),
     "location": "SIN", "cost": 300.00, "confirmed": True, "code": "SQ116-123", "notes": "Economy"},
    {"type": "Hotel", "description": "Siam Kempinski Bangkok",
     "start": now + timedelta(days=8, hours=12), "end": now + timedelta(days=10, hours=12),
     "location": "Bangkok", "cost": 450.00, "confirmed": True, "code": "SK-789", "notes": "Executive Room"},
]
create_trip_with_details(
...
print("   - Trips: 3 (Draft, Approved, Final)")
print("   - Each trip has 2 stops and 2 itinerary items.")
print("   - Memberships and custom categories added.")
print("=" * 60)
